...
void NS9542_Starting(BandType band, uint16_t freq, uint8_t clk_sel)
{
    ...
    //  NS9542_Audio_Output_setting();
    //  ドキュメントに記載なし、どう実装していいのか分からない??
...
void NS9542_find_pg(uint8_t ialgn, uint8_t *fine_phase, uint8_t *fine_gain,
                    uint8_t *result_pg)
{
    uint8_t i, j;
...
void NS9542_band_setting(BandType band)
{
    ...
    {
        case AM:
            NS9542_RegBitMaskSet(0x00, _BV(4) | _BV(5) | _BV(6),
                                 _BV(5)); // BAND
            NS9542_RegBitMaskSet(0x0C, _BV(0) | _BV(1) | _BV(2) | _BV(3),
                                 0); // AMFSEL
...
void NS9542_freq_setting(uint16_t psy, uint8_t clk_sel)
{
    if (clk_sel == 0)
    {
        ...
        NS9542_RegBitMaskSet(0x10, _BV(0) | _BV(1) | _BV(2) | _BV(3),
                             0); // CLK_SEL
    }
    else
    {
