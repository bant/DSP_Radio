...
#define IR_INIT_TIMER()                                                        \
    TCCR1B = 0b10000010 /* Initialize Timer (Timer1 for transmission/reception \
                           timing: Free running, clk/8) */

#if F_CPU == 8000000UL
#define IR_INIT_XMIT() /* Tx: Initialize Transmitter (Timer0 for IR \
                          subcarrier: Fast PWM, clk/8) */           \
    OCR0B = 8;                                                      \
...
#define IR_TX_38K() \
    OCR0A = 26;     \
    TCNT0 = 0 /* Tx: Set IR burst frequency to 38kHz */
#define IR_TX_40K() \
    OCR0A = 25;     \
    TCNT0 = 0 /* Tx: Set IR burst frequency to 40kHz */
#elif F_CPU == 10000000UL
#define IR_TX_38K() \
    OCR0A = 32;     \
    TCNT0 = 0 /* Tx: Set IR burst frequency to 38kHz */
...
#define IR_TX_ON() \
    TCCR0A |=      \
        _BV(COM0B1) /* Tx: Start IR burst 比較一致でOC0Bピン Lowレベル出力 */
#define IR_TX_OFF() TCCR0A &= ~_BV(COM0B1) /* Tx: Stop IR burst */
#define IR_TX_TEST() \
    TCCR0A& _BV(COM0B1) /* Tx: Check if IR is being transmitted or not */

#define IR_CAPT_TEST()                                                         \
    TCCR1B& _BV(ICES1) /* Rx: Check which edge generated the capture interrupt \
                          */
#define IR_CAPT_RISE() \
    TCCR1B |= _BV(ICES1) /* Rx: Set captureing is triggered on rising edge */
#define IR_CAPT_FALL()                                                       \
    TCCR1B &= ~_BV(ICES1) /* Rx: Set captureing is triggered on falling edge \
                             */

#define IR_CAPT_ENA() \
    TIFR = _BV(ICF1); \
    TIMSK = _BV(ICIE1)     /* Rx: Enable captureing interrupt */
#define IR_CAPT_REG() ICR1 /* Rx: Returns the value in capture register */
#define IR_CAPT_DIS() \
    TIMSK &= ~_BV(ICIE1) /* Tx && Rx: Disable captureing interrupt */

#define IR_COMP_ENA(n)   \
    OCR1A = TCNT1 + (n); \
    TIFR = _BV(OCF1A);   \
    TIMSK |= _BV(OCIE1A) /* Enable compare interrupt n count after now */
#define IR_COMP_DIS() TIMSK &= ~_BV(OCIE1A) /* Disable compare interrupt */
#define IR_COMP_NEXT(n) \
    OCR1A += (n) /* Tx: Increase compare register by n count */
...
#define IR_CAPT_ENA()  \
    TIFR1 = _BV(ICF1); \
    TIMSK1 = _BV(ICIE1)    /* Rx: Enable captureing interrupt */
#define IR_CAPT_REG() ICR1 /* Rx: Returns the value in capture register */
#define IR_CAPT_DIS() \
    TIMSK1 &= ~_BV(ICIE1) /* Tx && Rx: Disable captureing interrupt */
...
ISR_CAPTURE()
{
    ...
    if (IR_CAPT_TEST())
    {
        ...
        pw2 = ct;
        if (IR_USE_SONY && IrCtrl.fmt == SONY &&
            pw1 >= (uint16_t)(T_SONY * 0.8) && pw1 <= (uint16_t)(T_SONY * 2.5))
        {
            i = IrCtrl.phase / 8;
    ...
    if (IrCtrl.state >= IR_RECVED)
        return; /* Reject if not ready to receive */

    f = 0;
    if (IR_USE_NEC && pw >= T_NEC * 13 &&
        pw <= T_NEC * 19) /* Is NEC leader pattern? */
    {
        if (ct >= T_NEC * 6 && ct <= T_NEC * 10)
            f = NEC;
        if (ct >= T_NEC * 3 && ct <= T_NEC * 5)
            f = NEC | REPT;
    }
    if (IR_USE_AEHA && pw >= T_AEHA * 5 &&
        pw <= T_AEHA * 12) /* Is AEHA leader pattern? */
    {
        if (ct >= (uint16_t)(T_AEHA * 2.5) && ct <= (uint16_t)(T_AEHA * 5.5))
            f = AEHA;
        if (ct >= T_AEHA * 5 && ct <= T_AEHA * 11)
            f = AEHA | REPT;
    }
    if (IR_USE_SONY && pw >= T_SONY * 3 &&
        pw <= T_SONY * 5) /* Is SONY leader pattern? */
    {
        if (ct >= (uint16_t)(T_SONY * 0.75) && ct <= (uint16_t)(T_SONY * 1.25))
            f = SONY;
    }
    ...
    if (IrCtrl.state == IR_RECVING)
    {
        ...
        d = IrCtrl.buff[i];
        f = IrCtrl.fmt;
        if (IR_USE_NEC && f == NEC && pw <= (uint16_t)(T_NEC * 1.5) &&
            ct <= (uint16_t)(T_NEC * 3 * 1.5)) /* Is NEC data mark? */
        {
        ...
        if (IR_USE_AEHA && f == AEHA && pw <= (uint16_t)(T_AEHA * 1.5) &&
            ct <= (uint16_t)(T_AEHA * 3 * 1.5)) /* Is AEHA data mark? */
        {
        ...
        if (IR_USE_SONY && f == SONY &&
            ct <= (uint16_t)(T_SONY * 1.5)) /* Is SONY data mark? */
        {
            return; /* Nothing to do at start of carrier */
        }
    }

    IrCtrl.state = IR_IDLE; /* When an invalid mark width is detected, abort and
                               return idle state */
}
#endif /* IR_USE_RCVR */
...
#if IR_USE_XMIT
int IR_xmit(uint8_t fmt,         /* Frame format: NEC, AEHA or SONY */
            ...
            )
{
    ...
    switch (fmt)
    {
        ...
        case AEHA: /* AEHA frame */
            if ((len > 0 && len < 48) || len % 8)
                return 0;    /* Must be 48 bit or longer data */
        ...
        case SONY: /* SONY frame */
            if (len != 12 && len != 15 && len != 20)
                return 0;    /* Must be 12, 15 or 20 bit data */
